...
// @desc    Generate AI-powered blog posts manually
// @access  Private (Admin only)
router.post('/generate-content', auth, authorize('admin', 'super_admin'), async (req, res) => {
  try {
    ...
    // Use the content scheduler for generation
    const results = await contentScheduler.triggerManualGeneration({
      count,
      topics,
      categories,
      generateImages,
    ...
    res.json({
      success: true,
      message: `Generated ${results.successful} out of ${count} blog post(s)`,
      data: {
        postsGenerated: results.successful,
    ...
    logger.error('Manual content generation error:', error);
    res.status(500).json({
      success: false,
      message: error.message
    });
...
// @route   GET /api/admin/generation-status
// @desc    Get content generation status and logs
// @access  Private (Admin only)
router.get('/generation-status', auth, authorize('admin', 'super_admin'), async (req, res) => {
  try {
    const { limit = 50 } = req.query;
    
    const status = contentScheduler.getStatus();
    const logs = contentScheduler.getLogs(parseInt(limit));
...
// @desc    Start the automatic content scheduler
// @access  Private (Admin only)
...
// @desc    Stop the automatic content scheduler
// @access  Private (Admin only)
...
// @route   DELETE /api/admin/generation-logs
// @desc    Clear generation logs
// @access  Private (Admin only)
router.delete('/generation-logs', auth, authorize('admin', 'super_admin'), async (req, res) => {
  try {
    contentScheduler.clearLogs();
    
    res.json({
      success: true,
      message: 'Generation logs cleared'
    });
  } catch (error) {
